...
namespace data_structures
{
	...
	public:
		...
		bool deleteNode(node *toDelete){ // Gets a node with the given data
	...
	// private:
		...
		void copyNode(node *thisNode, const node *otherNode){
			if(thisNode==nullptr){//if it's a root node
				thisNode=new node;
				root=thisNode;
			}
	...
	public:
		...
		RedBlackTree(RedBlackTree &&other){
			this->root=other->root;//just transfer toot node
			other->root=nullptr;
		...
		bool remove(dataPoint &value){  // Deletes a node with the given data
			node* temp =getNode(value);
			if(!temp){
				return false;
			}
